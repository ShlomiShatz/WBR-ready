...
wbr914_minimal::wbr914_minimal()
  : _tioChanged( false ),
    _stopped( true ), _motorsEnabled( false ), _debug(0)
{
...
int wbr914_minimal::GetAnalogSensor(int s, short * val )
{
  ...
  // The following code is from the original driver and has been commented for simplicity.
  // *************
  ...
  // The following 2 lines are written by me, can be replaced with the above
  *val = (uint16_t)BytesToInt16(&(ret[2]));
  return 0;
...
void wbr914_minimal::GetIRData(float *d)
{
  ...
  for (uint32_t i=0; i < NUM_IR_SENSORS; i++)
  {
    ...
    // The following code is from the original driver and has been commented for simplicity. It containes conversions of data from IR to meters
    // ************
    ...
    // The following line is written by me, returns the pure voltage data measured from the IR sensors (yellow wire). Can be replaced with the above
    d[i] = (float)val / 10000.0;
  }
}
